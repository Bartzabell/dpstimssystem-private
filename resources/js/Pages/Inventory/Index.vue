        ...
        <Modal :show="isFormVisible" @close="!isFormVisible" class="fixed inset-0 z-50">
            <div class="w-[50vw]" v-if="isFormVisible">
                <div
                    ...
                    <div>
                        <h1 class="text-2xl font-extrabold dark:text-gray-200">Product Registry</h1>
                    </div>
                ...
                <form @submit.prevent="submit">
                    <div class="grid grid-cols-1 !text-[10px] 2xl:!text-sm gap-2 p-5 mt-10 lg:grid-cols-2">
                        <div class="col-span-1 lg:col-span-2">
                            <CustomInput name="Product Code" v-model="form.item_code" disabled />
                        </div>
                        <CustomInput name="Name" v-model="form.name" />
                        <CustomInput name="Type" v-model="form.type" />
                        <div>
                            <label
                                class="!text-[8px] lg:!text-[10px] font-medium 2xl:!text-sm dark:text-gray-200">Material</label>
                            <SearchableDropdown class="border rounded-lg border-slate-600" v-model="selectedMaterial"
                                :items="materials" placeholder="Search Material..."
                                @change="form.material = $event.name" />
                        </div>
                        <div>
                            <label
                                class="!text-[8px] lg:!text-[10px] font-medium 2xl:!text-sm dark:text-gray-200">Category</label>
                            <SearchableDropdown class="border rounded-lg border-slate-600" v-model="selectedCategory"
                                :items="categories" placeholder="Search category"
                                @change="form.category = $event.name" />
                        </div>
                        <div>
                            <label
                                class="!text-[8px] lg:!text-[10px] font-medium 2xl:!text-sm dark:text-gray-200">Color</label>
                            <SearchableDropdown class="border rounded-lg border-slate-600" v-model="selectedColor"
                                :items="colors" placeholder="Search Colors..." @change="form.color = $event.name" />
                        </div>
                        <div class="grid grid-cols-2 gap-2">
                            <CustomInput name="Size" v-model="form.size" />
                            <div>
                                <label
                                    class="!text-[8px] lg:!text-[10px] font-medium 2xl:!text-sm dark:text-gray-200">UOM</label>
                                <SearchableDropdown class="border rounded-lg border-slate-600" v-model="selectedUom"
                        ...
                        <table
                                ...
                                <tr class="text-xs text-center text-white bg-gray-100 md:text-base">
                                    ...
                                    <th class="px-2 py-1 border bg-emerald-800 whitespace-nowrap">
                                        <button @click="sort('name')" class="flex items-center justify-center w-full">
                                            NAME
                                            <PhCaretUp v-if="sortField === 'name' && sortDirection === 'asc'"
                                                class="ml-1" :size="16" />
                                            <PhCaretDown v-if="sortField === 'name' && sortDirection === 'desc'"
                                                class="ml-1" :size="16" />
                                        </button>
                                    </th>
                                    <th class="px-2 py-1 border bg-emerald-800 whitespace-nowrap">
                                        <button @click="sort('item_code')"
                                            ...
                                            PRODUCT CODE
                                            <PhCaretUp v-if="sortField === 'item_code' && sortDirection === 'asc'"
                                                class="ml-1" :size="16" />
                                            <PhCaretDown v-if="sortField === 'item_code' && sortDirection === 'desc'"
                                                class="ml-1" :size="16" />
                                        </button>
                                    </th>
                                    <th class="px-2 py-1 border bg-emerald-800 whitespace-nowrap">
                                        <button @click="sort('category')"
                                            class="flex items-center justify-center w-full">
                                            CATEGORY
                                            <PhCaretUp v-if="sortField === 'category' && sortDirection === 'asc'"
                                                class="ml-1" :size="16" />
                                            <PhCaretDown v-if="sortField === 'category' && sortDirection === 'desc'"
                                                class="ml-1" :size="16" />
                                        </button>
                                    </th>
                                    <th class="px-2 py-1 border bg-emerald-800 whitespace-nowrap">
                                        <button @click="sort('material')"
                                            class="flex items-center justify-center w-full">
                                            MATERIAL
                                            <PhCaretUp v-if="sortField === 'material' && sortDirection === 'asc'"
                                                class="ml-1" :size="16" />
                                            <PhCaretDown v-if="sortField === 'material' && sortDirection === 'desc'"
                                                class="ml-1" :size="16" />
                                        </button>
                                    </th>
                                    <th class="px-2 py-1 border bg-emerald-800 whitespace-nowrap">
                                        <button @click="sort('color')" class="flex items-center justify-center w-full">
                                            COLOR
                                            <PhCaretUp v-if="sortField === 'color' && sortDirection === 'asc'"
                                                class="ml-1" :size="16" />
                                            <PhCaretDown v-if="sortField === 'color' && sortDirection === 'desc'"
                                                class="ml-1" :size="16" />
                                        </button>
                                    </th>
                                    <th class="px-2 py-1 border bg-emerald-800 whitespace-nowrap">
                                        <button @click="sort('size')" class="flex items-center justify-center w-full">
                                            SIZE
                                            <PhCaretUp v-if="sortField === 'size' && sortDirection === 'asc'"
                                                class="ml-1" :size="16" />
                                            <PhCaretDown v-if="sortField === 'size' && sortDirection === 'desc'"
                                                class="ml-1" :size="16" />
                                        </button>
                                    </th>
                                    <th class="px-2 py-1 border bg-emerald-800 whitespace-nowrap">
                            ...
                            <tbody>
                                <tr class="text-xs text-gray-600 dark:text-gray-50 dark:bg-gray-500 dark:even:bg-gray-600 dark:hover:bg-gray-800 md:text-base hover:bg-blue-100 even:bg-gray-50"
                                    v-for="inventory in inventories.data" :key="inventory.id">
                                    ...
                                    <td class="px-2 py-1 border whitespace-nowrap">
                                        <div class="inline-flex justify-center w-full h-full gap-2 ">
                                            <button @click="edit(inventory)"
                                                class="p-2 text-white bg-blue-500 rounded-full">
                                                <PhPencil :size="16" />
                                            </button>
                                            <button @click="confirmDelete(inventory.id)"
