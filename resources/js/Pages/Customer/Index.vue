...
import { PhWarning, PhFilePlus, PhFloppyDisk, PhTrash, PhPencil, PhListMagnifyingGlass, PhDownloadSimple, PhX, PhArchive } from "@phosphor-icons/vue";
...
<template>
    <AppLayout title="Customer">
        ...
        <Modal :show="isFormVisible" @close="isFormVisible = false" class="fixed inset-0 z-50">
            <div class="w-[40vw]" v-if="isFormVisible">
                <div
                ...
                <form @submit.prevent="submit" class="pb-4 m-3 bg-white rounded shadow dark:bg-gray-800">
                    <div class="grid grid-cols-1 gap-5 p-5 mt-10 lg:grid-cols-2">
                        ...
                        <div class="flex items-center justify-end w-full">
                            <div class="flex items-center justify-center gap-2 mt-2">
                                ...
                                <ButtonCode v-if="editing" type="button" color="bg-gray-500 hover:bg-gray-700"
                                    text="Cancel" @click="cancelForm" />
                            </div>
                        </div>
                    </div>
                </form>
        ...
        <div class="p-5">
            <div class="p-6 mt-2 bg-white rounded shadow dark:bg-gray-700">
                <div class="flex flex-col items-end justify-end gap-2 mb-4 md:items-center md:flex-row">
                    <ButtonCode @click="toggleFormVisibility" text="Add Customer" :icon="PhFilePlus"
                ...
                <div v-if="customers && customers.data && customers.data.length > 0">
                    <div class="overflow-x-auto border rounded-lg dark:border-gray-600">
                        <table class="min-w-full divide-y divide-gray-200 dark:border-gray-600 dark:divide-gray-600">
                            ...
                            <tbody>
                                <tr class="text-xs text-gray-600 dark:text-gray-50 md:text-base dark:hover:bg-gray-800 hover:bg-blue-100 dark:bg-gray-500 dark:even:bg-gray-600 even:bg-gray-50"
                                    ...
                                    <td class="px-2 py-1 border whitespace-nowrap">{{ customer.street }} {{
                                        customer.municipality }} {{ customer.city }}</td>
                                    <td class="px-2 py-1 border whitespace-nowrap">
                                        <div class="inline-flex justify-center w-full h-full gap-2 ">
                                            ...
                                            <button @click="confirmDelete(customer.id)"
                                                class="p-2 text-white bg-orange-500 rounded-full">
                                                <PhArchive :size="16" />
                                            </button>
                                        </div>
                                    </td>
