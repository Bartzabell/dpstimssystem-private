...
const deleteItem = (id) => {
    if (confirm('Are you sure?')) {
        form.delete(route('supplier.destroy', id));
    }
};
                ...
                <form @submit.prevent="submit" class="pb-4 m-3 bg-white rounded shadow">
                    ...
                    <div class="grid grid-cols-1 gap-5 p-5 md:grid-cols-2">
                        ...
                        <div class="flex items-center justify-end w-full">
                            <div class="w-max">
                                <ButtonCode type="submit" :icon="editing ? PhFloppyDisk : PhFilePlus" color="bg-blue-500 hover:bg-blue-700" :text="editing ? 'Update Supplier' : 'Add Supplier'" />
                            </div>
                        </div>
                    </div>
                </form>
                        ...
                        <tr class="text-xs text-gray-600 md:text-base hover:bg-blue-100 even:bg-gray-50" v-for="supplier in suppliers.data" :key="supplier.id">
                            ...
                            <td class="px-2 py-1 border whitespace-nowrap">
                                <div class="inline-flex justify-center w-full h-full gap-2 ">
                                    ...
                                    <button @click="deleteItem(supplier.id)" class="p-2 text-white bg-red-500 rounded-full"><PhTrash :size="16" /></button>
                                </div>
                            </td>
                        </tr>
                    </tbody>
